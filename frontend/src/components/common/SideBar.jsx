...
const SideBar = () => {
  ...
  return (
    <div className='md:flex-[2_2_0] w-18 max-w-52'>
        <div className='sticky top-0 h-screen flex flex-col border-r border-gray-700 w-20 md:w-full'>
            ...
            <ul className='flex flex-col gap-3 mt-4'>
                <li className='flex justify-center md:justify-start'>
                    <Link to="/" className='flex gap-3 items-center hover:bg-stone-900 transition-all rounded-full duration-300 py-2 pl-2 pr-4 max-w-fit cursor-pointer'>
                        <MdHomeFilled className='s-8 h-8'/>
                ...
                <Link to={`/profile/${authUser.username}`} className='mt-auto mb-10 flex gap-2 items-start transition-all duration-300 hover:bg-[#181818] py-2 px-4 rounded-full'>
                    ...
                    <div className='flex justify-between flex-1'>
                        <div className='hidden md:block'>
                            <p className='text-white font-bold text-smw-20 truncate'>{authUser?.fullName}</p>
